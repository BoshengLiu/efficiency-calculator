...
这是本人的个人作品集，用于展示自动化开发能力。欢迎访问我的网站：[你的网站链接]
...
- **联系方式、品牌、案例、Banner 数字、CTA 文案**等对外展示内容的修改位置与步骤见 **[DESIGN.md](DESIGN.md)** 中的「对外展示内容修改指南」。
- **部署**：整站可当静态站部署（如 Vercel、Netlify、GitHub Pages）。若需保留统计，需能运行 `python server.py` 并保证 `data/` 可写；纯静态托管下看板无数据。

## 文档

- **[DESIGN.md](DESIGN.md)**：项目概述、功能模块、目录结构、开发指南、统计与日志、安全与部署、**对外展示内容修改指南**（含行号与示例）。
